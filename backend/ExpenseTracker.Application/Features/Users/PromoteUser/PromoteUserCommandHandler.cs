...
) : IHandler<PromoteUserCommand, OperationResult<object>>
{
    public async Task<OperationResult<object>> Handle(PromoteUserCommand message, CancellationToken cancellationToken = default)
    {
        ...
        UserRole newRole = targetUser.Role switch
        {
            ...
            UserRole.Admin => throw ResponseCatalog.User.AlreadyTopRole.ToException(),
            UserRole.SuperAdmin => throw ResponseCatalog.Authorization.CannotPromoteSuperAdmin.ToException(),
            _ => throw ResponseCatalog.User.AlreadyTopRole.ToException()
        };
